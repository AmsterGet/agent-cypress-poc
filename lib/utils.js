...
const isParallelRun = (reporterOptions) => reporterOptions.parallel && reporterOptions.autoMerge;
...
const getLaunchStartObject = (config) => {
  ...
  if (isParallelRun(config.reporterOptions)) {
    launchAttributes.push({ value: process.env.CI_BUILD_ID });
  }
