...
class CypressReporter extends Mocha.reporters.Base {
  ...
  static calcTotalLaunches() { // recalculate for parallel run (investigate how to get real specs quantity)
    ...
    console.log('Is parallel run: ', isParallelRun(CypressReporter.reporterOptions));
    if (isParallelRun(CypressReporter.reporterOptions)) {
      CypressReporter.totalLaunches = 1; // temporary hard code. must be removed
      return;
    }
