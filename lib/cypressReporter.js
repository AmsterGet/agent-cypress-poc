...
class CypressReporter extends Mocha.reporters.Base {
  constructor(runner, config) {
    super(runner);
    console.log('Runner: ', runner.suite);
    console.log('---------------------------------------------------------------');
    console.log(this);
    this.runner = runner;
    ...
    this.runner.on(EVENT_RUN_END, () => {
      ...
      if (CypressReporter.shouldStopLaunch()) {
        ...
        const finishLaunchIfNeeded = (prevCurrentLaunch) => {
          console.log('Current launch: ', CypressReporter.currentLaunch);
          console.log('Prev current launch: ', prevCurrentLaunch);
          if (CypressReporter.currentLaunch === prevCurrentLaunch) {
            this.worker.send({ event: EVENT_RUN_END, launch: getLaunchStartObject(config) });
          }
        };
        setTimeout(() => finishLaunchIfNeeded(CypressReporter.currentLaunch), 3000)
      }
    });
  ...
  static calcTotalLaunches() {
    console.log('Cypress config (calc total launches): ', CypressReporter.cypressConfig);
