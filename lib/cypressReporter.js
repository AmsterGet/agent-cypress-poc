...
class CypressReporter extends Mocha.reporters.Base {
  constructor(runner, config) {
    ...
    process.env.RP_CURRENT_LAUNCH = CypressReporter.currentLaunch;
    ...
    this.runner.on(EVENT_RUN_END, () => {
      CypressReporter.calcTotalLaunches();
      console.log('Total launches on finish: ', CypressReporter.totalLaunches);
      const parallel = isParallelRun(CypressReporter.reporterOptions);
      if (CypressReporter.shouldStopLaunch()) {
        this.worker.send({ event: EVENT_RUN_END, launch: getLaunchStartObject(config) });
      } else if (parallel) {
        process.env.RP_PREV_CURRENT_LAUNCH = CypressReporter.currentLaunch;
        console.log('Current launch: ', process.env.RP_CURRENT_LAUNCH);
        console.log('Prev current launch: ', process.env.RP_PREV_CURRENT_LAUNCH);
        const MAX_TIMEOUT = 37000;
        const CHECK_IN_PROGRESS_INTERVAL = 12000;
        const beginCheckTime = Date.now();
        const checkInterval = setInterval(() => {
          console.log('Current launch: ', process.env.RP_CURRENT_LAUNCH);
          console.log('Prev current launch: ', process.env.RP_PREV_CURRENT_LAUNCH);
          if (process.env.RP_CURRENT_LAUNCH === process.env.RP_PREV_CURRENT_LAUNCH) {
            this.worker.send({ event: EVENT_RUN_END, launch: getLaunchStartObject(config) });
            clearInterval(checkInterval);
          } else if (Date.now() - beginCheckTime > MAX_TIMEOUT) {
            clearInterval(checkInterval);
            console.log(`Finish launch error. Timeout of ${MAX_TIMEOUT}ms exceeded.`);
          }
        }, CHECK_IN_PROGRESS_INTERVAL);

        // const finishLaunchIfNeeded = (prevCurrentLaunch) => {
        //   console.log('Current launch: ', process.env.RP_CURRENT_LAUNCH);
        //   console.log('Prev current launch: ', prevCurrentLaunch);
        //   if (process.env.RP_CURRENT_LAUNCH === prevCurrentLaunch) {
        //     this.worker.send({ event: EVENT_RUN_END, launch: getLaunchStartObject(config) });
        //   }
        // };
        // setTimeout(() => finishLaunchIfNeeded(CypressReporter.currentLaunch), 10000);
      }
    });
...
process.env.RP_CURRENT_LAUNCH = 0;
process.env.RP_PREV_CURRENT_LAUNCH = 0;
