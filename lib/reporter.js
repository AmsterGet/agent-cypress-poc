...
class Reporter {
  ...
  runStart(launchObj) {
    console.log('Config from start launch', this.config);
  ...
  async mergeParallelLaunches() {
    ...
    if (!ciBuildId) {
      console.error('CI_BUILD_ID must not be empty');
      return;
    }
    ...
    try {
      ...
      request.attributes = (this.config.reporterOptions.attributes || []).concat({
        value: ciBuildId,
      });
