...
class Reporter {
  constructor(config) {
    ...
    this.client = new RPClient({ ...config.reporterOptions, debug: true }, agentInfo);
  ...
  runEnd() {
    const finishLaunchPromise = this.client
      ...
      .then(() => {
        console.log('Config from finish launch: ', this.config);
        if (!isParallelRun(this.config.reporterOptions)) {
          return;
        }
  ...
  async mergeParallelLaunches() {
    ...
    await this.client.restClient.create(mergeURL, request, { headers: this.headers });
